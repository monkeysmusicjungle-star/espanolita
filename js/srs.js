...
const State = {
  ...
  defaults(){
    return {
      onboarded: false,
      lang: "en",              // translation language: "en" | "nl"
...
const SRS = {
  // Ensure an item exists in the scheduler.
  ensure(id){
  ...
  // grade: true = correct, false = wrong
  answer(id, correct){
    ...
    if (correct){
      ...
      it.lapses++;
      it.ivl = 0;                       // comes back in this same session / today
  ...
  // Words that struggle the most (for the "weak words" list).
  weakest(n = 5){
    return Object.entries(State.data.items)
  ...
  // Adaptive band movement based on the last 30 answers.
  recentAccuracy(){
  ...
  // Pick the next new vocab items to introduce (from current band, in order).
  nextNewWords(n){
    const d = State.data;
    ...
    while (out.length < n && band < bands.length){
      ...
      if (out.length < n) band++;   // current band exhausted → borrow from next
    }
    return out;
  },
  ...
  introduce(id){
    ...
    this.ensure(id);
    // Auto-advance the band once ~85% of it has been unlocked.
    const bands = ["A1", "A2", "B1"];
    if (d.band < 2){
